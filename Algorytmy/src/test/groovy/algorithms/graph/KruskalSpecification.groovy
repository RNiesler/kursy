...
class KruskalSpecification extends Specification {
    def 'test on empty'() {
        setup:
        Supplier<Graph<String>> supplier = { new LinkedGraph<>() }
        final undirectedGraph = UndirectedGraph.of(supplier)
        when:
        def tree = new Kruskal<String>(undirectedGraph).minimumSpanningTree
        then:
        tree.empty
    }
    ...
    def 'test on linked'() {
        ...
        when:
        def minimumSpanningTree = new Kruskal(undirectedGraph).minimumSpanningTree
        then:
